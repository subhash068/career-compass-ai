...
class LearningService:
    ...
    def complete_step(
        ...
        Also updates user's current_role to the completed role title.
        """
        path = db.query(LearningPath).filter_by(
            id=path_id,
            user_id=user_id
        ...
        if path.progress == 100 and old_progress < 100:
            ...
            # Auto-update user's current_role to the completed role title
            if path.target_role:
                role_title = path.target_role.title or path.target_role.name
                if role_title:
                    user = db.query(User).filter_by(id=user_id).first()
                    if user:
                        user.current_role = role_title
                        db.commit()
