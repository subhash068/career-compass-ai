...
class EmbeddingService:
    ...
    def get_model(model_name: Optional[str] = None):
        """Get or initialize the embedding model with retry + fallback."""
        ...
        if EmbeddingService._model is None or EmbeddingService._model_name != model_name:
            ...
            if AISettings.HF_HUB_OFFLINE:
                print("⚠️ HF_HUB_OFFLINE=1 detected. Using fallback embedding.")
                EmbeddingService._model = get_fallback_model()
                EmbeddingService._model_name = "fallback-tfidf-svd"
                return EmbeddingService._model

            # Try sentence-transformers with retry logic
            max_retries = AISettings.EMBEDDING_RETRY_COUNT
            timeout = AISettings.EMBEDDING_TIMEOUT
            retry_delay = 1.0  # Start with 1s backoff

            last_exception = None
            for attempt in range(max_retries):
                try:
                    print(f"Loading model attempt {attempt + 1}/{max_retries} (timeout={timeout}s)...")
            ...
            if AISettings.EMBEDDING_USE_FALLBACK:
                print("🔄 Using TF-IDF fallback embedding service")
                EmbeddingService._model = get_fallback_model()
                EmbeddingService._model_name = "fallback-tfidf-svd"
                ...
                raise RuntimeError(
                    f"Failed to load '{model_name}' after {max_retries} retries. "
                    f"Set EMBEDDING_USE_FALLBACK=true for TF-IDF fallback. "
