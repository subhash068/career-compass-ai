...
# Enhanced intent patterns with admin commands
    INTENT_PATTERNS = {
        "admin": [
            r"\b(create|add|new)\s+(user|skill|domain|question|quiz)\b",
            r"\b(list|show|display|get)\s+(users?|skills?|domains?)\b",
            r"\b(delete|remove)\s+(user|skill|domain)\b",
            r"\b(update|modify|change)\s+(user|skill)\b.*role",
            r"\b(admin|administrator|management)\b.*\b(command|action)\b"
        ],
        "career": [
