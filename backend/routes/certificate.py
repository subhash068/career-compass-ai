...
# ============== Certificate Metadata (MUST be before generic /{certificate_id}) =============

@router.get("/{certificate_id}/metadata")
def get_certificate_metadata(
    ...
    """
    Export certificate metadata as JSON.
    Returns metadata in a standardized format for easy verification and import.
    """
    try:
        certificate = db.query(Certificate).filter(
            Certificate.id == certificate_id,
        ...
        # Map performance grade to descriptive grade
        grade_mapping = {
            "A+": "Distinction",
            "A": "Distinction",
            "B": "Excellent",
        ...
        descriptive_grade = grade_mapping.get(str(certificate.performance_grade), "Completed")
        
        # Generate verification URL
        verification_url = certificate.verification_url or f"https://careercompass.ai/verify/{certificate.certificate_unique_id}"
        
        # Build metadata JSON
        metadata = {
            "certificate_id": certificate.certificate_unique_id or f"CCA-LEGACY-{certificate.id}",
            "candidate_name": certificate.user_name,
            "course": certificate.role_title,
            "duration": certificate.course_duration or "120 Hours",
            "mode": certificate.completion_mode or "Online + Project Based",
            "skills": skills[:10],  # Limit to 10 skills
            "score": round(certificate.final_assessment_score, 1) if certificate.final_assessment_score else 0,
            "grade": descriptive_grade,
            "issued_on": certificate.issued_at.strftime("%Y-%m-%d") if certificate.issued_at else None,
            "issuer": "Career Compass AI",
            "verification_url": verification_url,
            "blockchain_hash": certificate.blockchain_hash or "",
            "certificate_hash": certificate.certificate_hash or "",
            "digital_signature": certificate.digital_signature or "",
            "blockchain_network": certificate.blockchain_network or "",
            "blockchain_tx_id": certificate.blockchain_tx_id or "",
            "expiry_date": certificate.expiry_date.strftime("%Y-%m-%d") if certificate.expiry_date else None,
            "is_verified": True,
            "issued_at_iso": certificate.issued_at.isoformat() if certificate.issued_at else None,
        }
...
# ============== Certificate Metadata JSON Export =============
