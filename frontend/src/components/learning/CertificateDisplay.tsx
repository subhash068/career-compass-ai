...
export function CertificateDisplay({ certificate, onClose }: CertificateDisplayProps) {
  ...
  // Download Certificate Metadata JSON
  const handleDownloadMetadata = async () => {
    setIsDownloadingMetadata(true);
    try {
      const metadata = await certificateApi.getCertificateMetadata(certificate.id);
      ...
      const link = document.createElement('a');
      link.href = url;
      link.download = `metadata-${certificate.certificate_unique_id || certificate.id}.json`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
        ...
        <CardContent className="p-0" style={{ background: 'transparent' }}>
          ...
          <div className="px-8 py-4 bg-white/90 backdrop-blur flex justify-center gap-3 mt-4 rounded-lg shadow-lg">
            ...
            <Button 
              ...
              title="Download Certificate Metadata JSON"
            >
